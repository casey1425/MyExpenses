...
`dotnet build`로 빌드를 확인했고, 별도의 임시 SQLite 데이터베이스에서 저장·수정·다시 읽기·개별 삭제·전체 삭제, 월·카테고리별 필터, 카테고리별 통계를 검증했습니다. 자동화된 테스트 프로젝트는 아직 저장소에 포함되지 않았습니다.

다음 후보 기능은 CSV 내보내기와 월별 예산 설정입니다.
