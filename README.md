...
- 이번 달 지출 합계 표시
- 개별 지출 삭제
- 전체 삭제: 삭제 건수와 주의 문구를 보여주는 확인 단계를 거친 뒤 실행
...
```text
Components/Pages/Home.razor      지출 입력·조회·삭제 화면과 이벤트 처리
...
`dotnet build`로 빌드를 확인했고, 별도의 임시 SQLite 데이터베이스에서 저장·다시 읽기·개별 삭제·전체 삭제를 검증했습니다. 자동화된 테스트 프로젝트는 아직 저장소에 포함되지 않았습니다.

다음 후보 기능은 지출 수정, 월별·카테고리별 필터, 카테고리별 통계, CSV 내보내기입니다.
