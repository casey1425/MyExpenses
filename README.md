...
- 카테고리별 지출 통계: 이번 달 또는 필터 결과의 금액·건수·비율을 막대그래프로 표시
- CSV 내보내기: 현재 조회 결과 또는 전체 기록을 날짜·금액·카테고리·메모 형식으로 다운로드
...
7. C#의 `GroupBy` 집계로 카테고리별 지출 통계를 추가했습니다.
8. C#으로 CSV 파일을 생성해 필터 결과와 전체 기록을 각각 내려받을 수 있게 했습니다.
...
Data/ExpenseStatistics.cs        카테고리별 C# 집계
Data/ExpenseCsvExporter.cs       UTF-8 CSV 생성과 텍스트 이스케이프
...
`dotnet build`로 빌드를 확인했고, 별도의 임시 SQLite 데이터베이스에서 저장·수정·다시 읽기·개별 삭제·전체 삭제, 월·카테고리별 필터, 카테고리별 통계를 검증했습니다. CSV는 한글·쉼표·따옴표·줄바꿈·수식으로 해석될 수 있는 메모와 빈 결과를 확인했습니다. 자동화된 테스트 프로젝트는 아직 저장소에 포함되지 않았습니다.

다음 후보 기능은 월별 예산 설정입니다.
